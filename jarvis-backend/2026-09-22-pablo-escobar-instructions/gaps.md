...
**Instruction files for the restructured seats (2026-09-21, updated 09-22).**
pablo-escobar.md has since landed, so the list below is now five files rather
than six: tony-soprano, christopher-moltisanti, silvio-dante, paulie-gualtieri,
and paulie-cicero. Pablo's file also brought two conflicts with how the runner
uses these files, neither changed because the text carries verbatim: its TOOLS
AND ACCESS grants Airtable and Google Sheets Write and post trial Stripe
actions, while the seat's runtime tools are airtable_list and stripe_list,
both read only, and the preamble states external tools are read only; and its
TALKING TO DAVE says decisions come as multi-choice widget options and never in
prose, which does not exist in a backend run where the approval queue is the
only channel. Tony holds both.

**The original entry, still true for the five remaining files.** The roster
now names tony-soprano (basecode), pablo-escobar, christopher-moltisanti,
...
a hook wake to pablo-escobar logs `event_skipped` with that message, and the
