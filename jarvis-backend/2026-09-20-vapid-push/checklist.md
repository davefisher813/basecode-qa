...
Commit: 4ff74b0 (the commit this change is built on)
Date: 2026-09-20
...
| 5 | Gate on the clean commit, publish | green, manual pass | filled after the run | |
| 6 | Set BOTH values in Railway in one change | one variable change, one redeploy | filled after | |
| 7 | Boot log after redeploy | `[PUSH] VAPID initialized`, no `init failed`, summary `✓ VAPID ready` | filled after | |
| 8 | `/api/health` after redeploy | `pushEnabled: true` | filled after | |
