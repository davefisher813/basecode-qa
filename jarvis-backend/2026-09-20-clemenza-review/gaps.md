...
|---|---|---|
| `server.js` | 24 | Text only, in log strings and comments. It is JARVIS code, so Dave gives the go. Any change that touches `server.js` for another reason now has to clean it first. |
| `sw.js` | 8 | The live Netlify service worker. Cleaning it means a redeploy of the old app, so it waits for a reason to redeploy. |

The number may never increase. It went 52, then 32, and the next move is
downward or nothing.
